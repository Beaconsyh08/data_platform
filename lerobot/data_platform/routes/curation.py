...
def register_curation_routes(app, ctx):
    ...
    def checked(handler):
        ...
        def wrapped(*args, **kwargs):
            if request.method not in {"GET", "HEAD"} and not request.path.startswith("/api/agents/"):
                denied = _role_denied("operator", "admin") if ctx.control_plane_store else None
                if denied:
                    return denied
            try:
    ...
    def capabilities(target, node, snapshot, info):
        user = _current_user()
        editable = ctx.control_plane_store is None or bool(user and user["role"] in {"operator", "admin"})
    ...
    def submit(body):
        ...
        denied = _role_denied("operator", "admin")
        if denied:
            return denied
    ...
    def complete(job, returned):
        ...
        if publication:
            ...
            if not actor or not actor.get("active") or actor["role"] not in {"operator", "admin"}:
