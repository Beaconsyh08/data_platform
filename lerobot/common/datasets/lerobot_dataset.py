...
def _column_to_numpy(column) -> np.ndarray:
    """Convert Torch, Arrow, or sequence-backed columns to NumPy without changing values."""
...
class LeRobotDatasetMetadata:
    ...
    def load_metadata(self):
        ...
        if self._version < packaging.version.parse("v2.1"):
            ...
            episode_ids = list(self.episodes_stats.keys())
            episode_stats = [self.episodes_stats[episode_id] for episode_id in episode_ids]
            try:
                ...
                logging.warning(
                    "Failed to aggregate episode stats for dataset '%s' at '%s'. "
                    "Retrying with malformed features skipped. Root cause: %s",
                    self.repo_id,
                    self.root,
                    exc,
                )
                self.stats = aggregate_stats(
                    episode_stats,
                    skip_mismatched_features=True,
                    sample_ids=episode_ids,
                    sample_label="episode_index",
                )
                fallback_stats = load_stats(self.root)
                if fallback_stats is not None:
                    # Use dataset-level stats only for features omitted from episode aggregation.
