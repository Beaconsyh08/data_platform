...
日常操作请先阅读 [当前环境构成与更新手册](data_platform_operations.md)，其中包含真实部署路径、
同步硬升级与先开发后生产的完整命令。本文保留首次安装、迁移和恢复的详细说明。
...
已生成发布包时，用原版本号和 `--release` 重试，不要再次使用 `--version` 构建同名包。
安装目录的 `manifest.sha256` 只在依赖安装及导入检查完成后写入。
如果中断留下缺少该文件的非当前版本目录，部署脚本将其移到同级
`.incomplete-<version>-<随机后缀>/installation` 保留现场，然后重新安装。
如果不完整目录正由 `current` 使用，或目录本身是符号链接，脚本停止并要求先检查现场。
发布包校验失败或已完成安装的摘要不匹配仍会停止，不会自动覆盖。
