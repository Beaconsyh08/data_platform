...
def test_hf_transform_rejects_invalid_image_bytes():
    batch = {"image": [{"bytes": b"not an image", "path": None}]}

    with pytest.raises(ValueError, match="Failed to decode image column 'image' item 0"):
        hf_transform_to_torch(batch)
